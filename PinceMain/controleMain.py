...
def is_hand_open(hand_landmarks):
    ...
    for tip in finger_tips:
        if calculate_distance(hand_landmarks.landmark[tip], palm_base) > calculate_distance(hand_landmarks.landmark[tip - 2], palm_base):
            open_fingers += 1

    return open_fingers >= 3  # Considérer la main ouverte si au moins 3 doigts sont ouverts
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                hand_type = "Main gauche" if is_left_hand(hand_landmarks) else "Main droite"
                if is_hand_open(hand_landmarks):
                    cv2.putText(frame, f'{hand_type} ouverte', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                else:
                    cv2.putText(frame, f'{hand_type} fermee', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
