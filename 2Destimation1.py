"""controle du robot avec vision 2D"""
import cv2
...
def affiche_texte2(image, angle, body, typ):
    cv2.putText(image, typ + str(angle), 
                           tuple(np.multiply(body, [640, 480]).astype(int)), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA
                                )
...
def convexite(p1, p2, p3):
    [x1, y1] = p1
    [x2, y2] = p2 
    [x3, y3] = p3 
    if x3 <= x2 and x2 <= x1 :
        m = (y3 - y2)/(x3 - x2) - (y2 - y1)/(x2 - x1) 
        return m > 0
    if x2 <= x3 and x3 <= x1 :
        m = (y2 - y3)/(x2 - x3) - (y3 - y1)/(x3 - x1)
        return m < 0
    if x1 <= x2 and x2 <= x3 :
        m = (y1 - y2)/(x1 - x2) - (y2 - y3)/(x2 - x3)
        return m > 0
    if x2 <= x1 and x1 <= x3 :
...
def convexite_bras_droit(landmarks):
    ...
    elbow = [landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].y]
    wrist = [landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].y]
    return convexite(shoulder, elbow, wrist)
    

longueurs = init_longueur()
print(longueurs)
qlongueurs = {'RightUpperArm': 0.28180931597450054, 'LeftUpperArm': 0.2412316313128073, 'RightLowerArm': 0.3757721842678538, 'LeftLowerArm': 0.3126503633940726}
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            landmarks = results.pose_landmarks.landmark
            sideAngle, frontalAngle, left_elbow = angle_left_elbow(landmarks, longueurs['LeftLowerArm'])
            x, y = left_elbow
        ...
        try:
            landmarks = results.pose_landmarks.landmark
            ElbSideAngle, frontalAngle, right_elbow= angle_right_elbow(landmarks, longueurs['RightLowerArm'])
            x, y = right_elbow
        ...
        try:
            landmarks = results.pose_landmarks.landmark   
            sideAngle, frontalAngle, left_shoulder= angle_left_shoulder(landmarks, longueurs['LeftUpperArm'])
            x, y = left_shoulder
        ...
        try:
            landmarks = results.pose_landmarks.landmark
            ShSideAngle, frontalAngle, right_shoulder= angle_right_shoulder(landmarks, longueurs['RightUpperArm'])
            x, y = right_shoulder
            ...
            goto(goal_positions=right_angled_position,duration=1.0,interpolation_mode=InterpolationMode.MINIMUM_JERK)   
        except:
            pass
