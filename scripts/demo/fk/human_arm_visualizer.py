...
from kinematics.fk.human_arm_fk import (
    dh_transformation_matrix,
    calculate_joint_positions,
)


def plot_coordinate_frame(ax, T, scale=0.01, label=None):
...
def visualize_robot(dh_params, joint_names=None, fig_size=(10, 8)):
    ...
    """
    # Get joint positions
    joint_positions = calculate_joint_positions(dh_params)
...
def create_interactive_robot(base_dh_params, joint_names):
    """Create an interactive visualization with sliders for joint angles."""
    # Initial parameters
    num_joints = len(base_dh_params)
    assert joint_names is not None and len(joint_names) == num_joints
    ...
    joint_positions = calculate_joint_positions(current_params)
    (robot_line,) = ax.plot(
        joint_positions[:, 0],
        joint_positions[:, 1],
        joint_positions[:, 2],
    ...
    def update(_):
        ...
        new_positions = calculate_joint_positions(current_params)
