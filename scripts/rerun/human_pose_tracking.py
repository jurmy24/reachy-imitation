...
DESCRIPTION = """
# Human pose shadowing with Reachy
This example uses Rerun to visualize the output of [MediaPipe](https://developers.google.com/mediapipe)-based tracking
of a human pose in 2D and 3D. Currently, it's only used to track the pose of the human in the video and log it to rerun. 
It does not yet make Reachy shadow the human.
""".strip()

EXAMPLE_DIR: Final = Path(os.path.dirname(__file__))
DATASET_DIR: Final = EXAMPLE_DIR / "dataset" / "pose_movement"
MODEL_DIR: Final = EXAMPLE_DIR / "model" / "pose_movement"
DATASET_URL_BASE: Final = (
    "https://storage.googleapis.com/rerun-example-datasets/pose_movement"
)
...
def track_pose(
    video_path: str, model_path: str, *, segment: bool, max_frame_count: int | None
) -> None:
        ...
        rr.AnnotationContext(
            rr.ClassDescription(
                ...
                keypoint_annotations=[
                    rr.AnnotationInfo(id=lm.value, label=lm.name)
    ...
    with closing(VideoSource(video_path)) as video_source:
        for idx, bgr_frame in enumerate(video_source.stream_bgr()):
            ...
            results = pose_landmarker.detect_for_video(
                mp_image, int(bgr_frame.time * 1000)
            )
            ...
            rr.log(
                "video/bgr",
                rr.Image(bgr_frame.data, color_model="BGR").compress(jpeg_quality=75),
            )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Uses the MediaPipe Pose solution to track a human pose in video."
    )
    parser.add_argument(
        "--video",
        type=str,
        default="arm_motion",
        choices=["backflip", "arm_motion"],
    ...
    parser.add_argument(
        "--no-segment", action="store_true", help="Don't run person segmentation."
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    track_pose(
        ...
        max_frame_count=args.max_frame,
    )
